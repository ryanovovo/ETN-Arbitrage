...
def state_to_embed(state):
    ...
    embed.add_field(name='最佳買價', value=f"{str(stock_frame['bid_price'][0])} ({str(stock_frame['bid_pct_chg'][0])}%)", inline=True)
    embed.add_field(name='最佳賣價', value=f"{str(stock_frame['ask_price'][0])} ({str(stock_frame['ask_pct_chg'][0])}%)", inline=True)
